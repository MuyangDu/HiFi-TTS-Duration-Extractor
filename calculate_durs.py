...
"""
Calculates durations for Hi-Fi TTS based on MFA TextGrid alignments.
"""
import argparse
import glob
import json
...
parser = argparse.ArgumentParser(description="Calculates phoneme durations for Hi-Fi TTS from TextGrids.")
...
def main():
    alignment_dir = os.path.join(args.hifitts_dir, 'alignments')

    # Create target_dir if necessary
    target_dir = os.path.join(args.hifitts_dir, 'phoneme_durations/')
    ...
    speakers = os.listdir(alignment_dir)

    for speaker in tqdm(speakers):
        speaker_dir = os.path.join(alignment_dir, speaker)
        books = os.listdir(speaker_dir)
        for book in books:
            book_dir = os.path.join(speaker_dir, book)
            textgrids = os.listdir(book_dir)
            for textgrid in textgrids:
                textgrid_path = os.path.join(book_dir, textgrid)
                basename = os.path.basename(textgrid)
                duration_basename = speaker + "_" + book + "_" + os.path.splitext(basename)[0] + ".pt"

                phones_mfa, tokens_mfa, durs = calculate_durations(textgrid_path, phone2idx)

                if phones_mfa is None:
                    oov_samples.append(textgrid)
                    continue

                target_path = os.path.join(target_dir, duration_basename)
    ...
    oov_target = os.path.join(args.hifitts_dir, 'wavs_to_ignore.pkl')
