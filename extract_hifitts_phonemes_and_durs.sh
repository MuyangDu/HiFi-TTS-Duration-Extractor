...
# Extracts the phonemes and alignments for the Hi-Fi TTS dataset via the
# Montreal Forced Aligner (MFA) library, and computes durations per phoneme.
# Assumes you have downloaded and expanded the Hi-Fi TTS dataset, and that they
# are located at the directory specified.
#
# This script will create:
# - <HIFITTS_BASE>/mappings.json: Contains word->phone and phone->idx mappings
# - <HIFITTS_BASE>/wavs_to_ignore.pkl: A pickled list of wavs to ignore
#     because of OOV words in their transcripts
# - <HIFITTS_BASE>/phoneme_durations/LJ*.npz: Numpy files for each wavfile
#     with fields 'tokens' (token indices) and 'durs' (durations per token)
#
# It will also create intermediate files:
# - <HIFITTS_BASE>/alignments/wavs/LJ*.TextGrid: MFA output
#
...
#
# Example Usage:
# ./extract_hifitts_phonemes_and_durs.sh /data/Hi-Fi TTS-1.1
...
Usage: $(basename "$0") [-h] \
          ...
          <HIFITTS_BASE>
Extracts phonemes and their respective durations for the HiFi-TTS dataset using
...
if [[ -z $1 ]]; then
  echo "Must specify a HiFi-TTS base directory."
  exit 1
fi
HIFITTS_BASE=$1
...
if [ -z $G2P_DICT ]; then
  if [ ! -f $HIFITTS_BASE/cmudict.dict ]; then
    echo "Downloading CMU dict."
    wget -P $HIFITTS_BASE $CMUDICT_URL
  fi
  if [ ! -f $HIFITTS_BASE/uncommented_cmudict.dict ]; then
    echo "Creating uncommented version of CMUdict."
    sed 's/\ \#.*//' $HIFITTS_BASE/cmudict.dict > $HIFITTS_BASE/uncommented_cmudict.dict
  fi
  G2P_DICT=$HIFITTS_BASE/uncommented_cmudict.dict
fi
...
mfa align --clean $HIFITTS_BASE/wavs $G2P_DICT english $HIFITTS_BASE/alignments

# Create JSON mappings from word to phonemes and phonemes to indices
echo "Creating word->phone and phone->idx mappings at $HIFITTS_BASE/mappings.json..."
